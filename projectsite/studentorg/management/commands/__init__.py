...
class Command(BaseCommand):
	...
	def handle(self, *args, **kwargs):
		# Ensure there is at least 1 College and Program before creating Students
		if not College.objects.exists():
			self.stdout.write(self.style.WARNING("No Colleges found. Please add some in admin first."))
			return
		if not Program.objects.exists():
			self.stdout.write(self.style.WARNING("No Programs found. Please add some in admin first."))
			return
	...
	def create_membership(self, count):
		...
		self.stdout.write(self.style.SUCCESS("Org memberships created."))
